...
Формат версии всегда содержит этап разработки:

    Alpha:   0.1.3-alpha
    Beta:    0.1.3-beta
    Release: 1.0.0-release

Запрещено использовать только числовой формат, например `0.1.3`.

Desktop и Server должны использовать единый источник версии из Core.

`Directory.Build.props` читает значение из `AppVersion.cs` для метаданных
сборок всех проектов. Не добавлять отдельные значения `<Version>` в `.csproj`.
Числовые AssemblyVersion и FileVersion формируются SDK автоматически;
полная версия приложения сохраняется в InformationalVersion.
